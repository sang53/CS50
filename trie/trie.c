...
// Function prototypes
bool check(char *word, node* currnode);
bool unload(void);
...
int main(int argc, char *argv[])
{
    ...
    if (check(get_string("Check word: "), root))
    {
        printf("Found!\n");
    }
    else
...
bool check(char* word, node *currnode)
{
    int currletter = tolower(*word) - 'a';
    if (currnode->is_word == true && *word == '\0')
    {
        return true;
    }
    else if (currnode->children[currletter] != NULL)
    {
        return check(&word[1], currnode->children[currletter]);
    }
    else
    {
        return false;
